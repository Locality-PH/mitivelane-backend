...
exports.getConversations = async (req, res) => {
  ...
  try {
    ...
    conversation.map((conversation, conversationIndex) => {
      var participants = conversation.participants.filter(
        (item) => item.uuid != userUuid
      );
      var count = 0;
      ...
      conversation.messages.map((messages, messagesIndex) => {
        ...
        if (conversation.messages.length == messagesIndex + 1) {
          finalValue[conversationIndex].time = new Date(
            messages.createdAt
          ).toLocaleTimeString();
        }
      });
